    ...
    <div className="group bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      ...
      <div className="relative h-56 overflow-hidden">
        ...
        <div className="absolute top-3 right-3 px-3 py-1 bg-white/95 backdrop-blur-sm rounded-full text-xs font-bold tracking-wide shadow-sm border border-gray-100 flex items-center gap-1 text-gray-700">
          <span className="text-brand-primary">👥</span> {displayCapacity} Occupancy
        </div>
      ...
      <div className="p-5">
        ...
        <div className="flex items-baseline gap-1 mb-4">
          <span className="text-2xl font-bold text-brand-primary">₹{displayPrice.toLocaleString()}</span>
        ...
        <div className="flex flex-wrap gap-2 mb-5 border-y border-gray-50 py-3">
          ...
          {amenities.length > 3 && (
            <div className="flex items-center text-xs font-medium text-brand-primary bg-brand-primary/10 px-2 py-1 rounded">
        ...
        <div className="grid grid-cols-2 gap-3">
          <Link to={`/property/${property._id}`} className="flex items-center justify-center gap-2 border border-brand-primary text-brand-primary py-2.5 rounded-xl font-semibold text-sm hover:bg-brand-bg transition-colors">
            View Details
          </Link>
          <button className="flex items-center justify-center gap-2 bg-brand-primary text-white py-2.5 rounded-xl font-semibold text-sm hover:bg-brand-secondary shadow-md hover:shadow-brand-dark/20 transition-all">
